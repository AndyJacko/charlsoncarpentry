...
<?php
if ($_GET["tbl_id"] != "") { 
  $_SESSION["id"] = $_GET["tbl_id"];
}
mysql_select_db($database_Wisdom_Mcr, $Wisdom_Mcr);
$query_RS = "SELECT * FROM tbl_gallery WHERE tbl_id=".$_SESSION["id"];
...
if (isset($_POST["upload"])) { 
  ...
  if((!empty($_FILES["image"])) && ($_FILES['image']['error'] == 0)) {
		...
		$insertSQL = sprintf("UPDATE tbl_gallery SET img_cat=".$_POST["img_cat"].",img_title='".$_POST["img_title"]."',img_caption='".$_POST["img_caption"]."' WHERE tbl_id=".$_SESSION["id"]);

    mysql_select_db($database_Wisdom_Mcr, $Wisdom_Mcr);
    $Result1 = mysql_query($insertSQL, $Wisdom_Mcr) or die(mysql_error());
	  unset($_SESSION['popo']);
		unset($_SESSION["id"]);
    header("location: gallery.php?c=".$_POST["img_cat"]);
  }
  if (strlen($error)==0){
...
if(strlen($large_photo_exists)>0 && strlen($thumb_photo_exists)>0){
  ...
  $insertSQL = sprintf("UPDATE tbl_gallery SET img_cat=".$_SESSION["img_cat"]." ,img_fname='".$usrCode.".jpg',img_title='".$_SESSION["img_title"]."',img_caption='".$_SESSION["img_caption"]."' WHERE tbl_id=".$_SESSION["id"]);
...
<div id="maincontainer">
  ...
  <div id="bodycontent">
    ...
    <h2>Upload Photo</h2>
    <br><br>
    <div class="aligncenter"><img src="/images/gallery/t/<?php echo $row_RS["img_fname"]; ?>" class="galborder"></div>
    ...
    <form name="photo" enctype="multipart/form-data" action="" method="post">
      ...
      <select name="img_cat" id="img_cat">
        <option value="0" <?php if($row_RS["img_cat"] == 0) { echo "selected"; } ?>>Extensions</option>
        <option value="1" <?php if($row_RS["img_cat"] == 1) { echo "selected"; } ?>>Lofts/Conversions</option>
        <option value="2" <?php if($row_RS["img_cat"] == 2) { echo "selected"; } ?>>Kitchens</option>
        <option value="3" <?php if($row_RS["img_cat"] == 3) { echo "selected"; } ?>>Bathrooms</option>
        <option value="4" <?php if($row_RS["img_cat"] == 4) { echo "selected"; } ?>>Wardrobes</option>
        <option value="5" <?php if($row_RS["img_cat"] == 5) { echo "selected"; } ?>>Garden</option>
        <option value="6" <?php if($row_RS["img_cat"] == 6) { echo "selected"; } ?>>Installations</option>
        <option value="7" <?php if($row_RS["img_cat"] == 7) { echo "selected"; } ?>>Storage</option>
        <option value="8" <?php if($row_RS["img_cat"] == 8) { echo "selected"; } ?>>Bespoke</option>
        <option value="9" <?php if($row_RS["img_cat"] == 9) { echo "selected"; } ?>>Something Different</option>
        <option value="10" <?php if($row_RS["img_cat"] == 10) { echo "selected"; } ?>>Work In Progress</option>
      </select>
      <label>Title<span class="small">Title of the photo.</span></label>
      <input name="img_title" type="text" id="img_title" size="50" onChange="checkuploadgalleryitems();" class="itemnorm" value="<?php echo $row_RS["img_title"]; ?>">
      <label>Caption<span class="small">A brief info of photo.</span></label>
      <textarea name="img_caption" cols="38" rows="5" id="img_caption" onChange="checkuploadgalleryitems();" class="itemnorm"><?php echo $row_RS["img_caption"]; ?></textarea>
